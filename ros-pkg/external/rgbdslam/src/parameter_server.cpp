...
void ParameterServer::defaultConfig() {
    ...
    config["publisher_queue_size"]          = static_cast<int> (1);                          // ROS standard parameter for all publishers
    ...
    config["max_keypoints"]                 = static_cast<int> (1000);                       // Extract no more than this many keypoints 
    config["min_keypoints"]                 = static_cast<int> (500);
    config["min_matches"]                   = static_cast<int> (25);                         //don't try RANSAC if less than this many matches (if using SiftGPU and GLSL you should use max. 60 matches)
    ...
    config["matching_candidates"]           = std::string("geodesic+sampling");
    ...
    config["predecessor_candidates"]        = static_cast<int> (10);                         // Compare Features to this many graph neighbours. Sample from the candidates
    config["neighbor_candidates"]           = static_cast<int> (10);                         // Compare Features to this many direct predecessors
    ...
    config["optimizer_iterations"]          = static_cast<int> (2);                          // maximum of iterations in online operation (i.e., does not affect the final optimization in batch mode). Optimizer stops after convergence anyway
    ...
    config["preserve_raster_on_save"]       = static_cast<bool> (false);                     //Filter NaNs when saving clouds
}
