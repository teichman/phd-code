...
namespace dst
{
  ...
  void OpticalFlowNode::_compute()
  {
    ...
    if(prev_img.rows == 0) {
      ...
      empty.points_ = NULL;
      empty.prev_pcd_indices_ = NULL;
      empty.pcd_indices_ = NULL;
      empty.status_ = NULL;
    ...
    for(size_t i = 0; i < points_.size(); ++i) {
      int x0 = min(curr_index.cols - 1, max(0, (int)points_[i].x));
	...
	continue;
      ...
      int max_x = min(curr_index.cols - 1, x0 + edge_radius_);
    ...
    // -- Get indices into point clouds for 3D flow.
    ROS_ASSERT(prev_pcd_indices_.empty());
    ROS_ASSERT(pcd_indices_.empty());
    ROS_ASSERT(prev_points_.size() == points_.size());
    prev_pcd_indices_.reserve(prev_points_.size());
    pcd_indices_.reserve(points_.size());
    cv::Mat1i prev_index = index_otl_->pull().previous_index_;
    for(size_t i = 0; i < prev_points_.size(); ++i) {
      prev_pcd_indices_.push_back(prev_index(prev_points_[i]));
      pcd_indices_.push_back(curr_index(points_[i]));
    }
    ...
    output_.prev_pcd_indices_ = &prev_pcd_indices_;
    output_.pcd_indices_ = &pcd_indices_;
