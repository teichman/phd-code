...
int main(int argc, char** argv)
{
  ...
  string video_path;
  vector<int> roi_values;
  opts_desc.add_options()
    ("help,h", "produce help message")
    ("video", bpo::value(&video_path)->required(), "Video to use")
    ("roi", bpo::value(&roi_values)->required()->multitoken(), "e.g. --roi width height x y")
    ;
  ...
  if(opts.count("help") || badargs) {
    cout << "Usage: " << argv[0] << " [OPTS] VIDEO --roi ROI" << endl;
    cout << endl;
    cout << opts_desc << endl;
    return 1;
  }
