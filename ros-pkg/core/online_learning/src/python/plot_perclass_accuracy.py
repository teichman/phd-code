...
basename = args.out + '-' + args.type + '-' + args.cl + '-accuracy'
...
# -- Load data
tmpfile = '.python-aastoeusstoehusathoesutahoneuth'
...
os.system("grep -A12 'Class: " + args.cl + "' `find -name 'track_results.txt' | grep test_results | sort` | grep Accuracy | awk '{print $NF}' > " + tmpfile)
...
if args.type == 'ann': 
    ax1.bar(num_unl, cl_num_ann_neg, width, color=(0.85, 0.85, 0.85))
    ax1.bar(num_unl, cl_num_ann_pos, width, color=(0, 0.3, 0), bottom=cl_num_ann_neg)
...
plot2 = ax2.plot(eval_num_unl + width, cl_accuracy, 'rv--', label='Accuracy')
ax2.set_ylabel('Accuracy')

# PR
#plot3 = ax2.plot(eval_num_unl, cl_precision, cp.linestyles[5], label='Precision')
#plot4 = ax2.plot(eval_num_unl, cl_recall, cp.linestyles[2], label='Recall')
