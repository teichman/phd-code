...
for test_name in test_names:
    pre_accs.append(100 * npLoadBash("grep 'Total acc' `find -L " + os.path.join(pre_dir, test_name) + " -name 'final_track_results.txt' | sort` | awk '{print $NF}'"))
    post_accs.append(100 * npLoadBash("grep 'Total acc' `find -L " + os.path.join(post_dir, test_name) + " -name 'final_track_results.txt' | sort` | awk '{print $NF}'"))

analyze(pre_accs, post_accs,
        'Pre', 'Post',
